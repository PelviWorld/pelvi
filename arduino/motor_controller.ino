...
void moveStep(const int axis, const unsigned long currentTime, const int detaTime, )
{
...
void checkMovementFinished(const int axis)
{
  ...
  } else if( axes[axis].stepsRemaining > 100 && digitalRead(axes[axis].minPin) == LOW)
  {
    axes[axis].currentSpeed = 0;
...
void performConcurrentMovements()
{
  ...
  for (const int axis = 0; axis < nrOfAxes; axis++)
  {
    if (axes[axis].stepsRemaining > 0)
    {
